...
del tabletestout.*
%pythonexe% -u ..\..\..\arcscripts\bin\sdnaintegral.py --dll %sdnadll% --im "net=zonetest;tables=zonetesttable.csv,zonetesttable2.csv" --om "net=tabletestout.shp" "linkonly;zonesums=lulinks=landuse*FULLlf@origzone,eucsum=euc@destzone;origweightformula=zoneweight*one*dztest*landuse*FULLlf/lulinks;destweightformula=eucsum;" >testout_table.txt 
%pythonexe% ..\..\..\arcscripts\shp2txt.py tabletestout >>testout_table.txt 
echo "duplicate zone table/sum test" >>testout_table.txt
%pythonexe% -u ..\..\..\arcscripts\bin\sdnaintegral.py --dll %sdnadll% --im "net=zonetest;tables=zonetesttable.csv" --om "net=tabletestout1.shp" "linkonly;zonesums=one=1@destzone" >>testout_table.txt 
echo "duplicate zone and net data test" >>testout_table.txt
%pythonexe% -u ..\..\..\arcscripts\bin\sdnaintegral.py --dll %sdnadll% --im "net=zonetest;tables=zonetesttableduplicate.csv" --om "net=tabletestout2.shp" "linkonly" >>testout_table.txt 
@echo on
